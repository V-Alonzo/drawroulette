...
import { HintText } from "@/app/components/InputFields/base/input/hint-text";
import { Label } from "@/app/components/InputFields/base/input/label";
    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled, isInvalid }) =>
                cx(
                    "group/input relative flex w-full flex-row place-content-center place-items-center rounded-lg bg-white shadow-xs ring-1 ring-zinc-300 transition-shadow duration-100 ease-linear ring-inset",

                    isFocusWithin && !isDisabled && "ring-2 ring-blue-500",
                    ...
                    isInvalid && "ring-red-300",
                    "group-invalid:ring-red-300",
        ...
        >
            ...
            {Icon && (
                <Icon className={cx("pointer-events-none absolute text-zinc-400", sizes[inputSize].iconLeading, context?.iconClassName, iconClassName)} />
            )}
            ...
            <AriaInput
                ...
                className={cx(
                    "m-0 w-full bg-transparent text-zinc-900 ring-0 outline-hidden placeholder:text-zinc-400 autofill:rounded-lg autofill:text-zinc-900 disabled:cursor-not-allowed",
            ...
            {tooltip && type !== "password" && (
                <Tooltip title={tooltip} placement="top">
                    <TooltipTrigger
                        className={cx(
                            "absolute cursor-pointer text-zinc-400 transition duration-100 ease-linear group-invalid/input:hidden hover:text-zinc-500 focus:text-zinc-500",
            ...
            {type === "password" && (
                <AriaButton
                    ...
                    className={cx(
                        "absolute flex cursor-pointer items-center justify-center text-zinc-400 transition duration-100 ease-linear hover:text-zinc-500 focus:text-zinc-500 focus:outline-hidden",
                        sizes[inputSize].iconTrailing,
                    )}
                >
            ...
            {shortcut && (
                <div
                    className={cx(
                        "pointer-events-none absolute inset-y-0.5 right-0.5 z-10 hidden items-center rounded-r-[inherit] bg-linear-to-r from-transparent to-white to-40% pl-8 md:flex",
                        sizes[inputSize].shortcut,
                    )}
                >
                    <span
                        aria-hidden="true"
                        className="pointer-events-none rounded px-1 py-px text-xs font-medium text-zinc-600 ring-1 ring-zinc-300 select-none ring-inset"
                    >
